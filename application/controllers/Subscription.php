...
class Subscription extends Home_Core_Controller {
    ...
    function video_transaction_history($param1 = '', $param2 = '') {
        if ($this->admin_is_login != 1)
            redirect(base_url(), 'refresh');
        /* start menu active/inactive section */
        $this->session->unset_userdata('active_menu');
        ...
        /* end menu active/inactive section */
        $data['page_name'] = 'video_transaction_history';
        $data['page_title'] = 'Video Transaction History';
        $this->db->order_by('id', "desc");
        $data['subscriptions'] = $this->db->get('videos_purchased')->result_array();
        $this->load->view('admin/index', $data);
    }
    ...
    function purchase($param1 = '', $param2 = '') {
        $data['page_name'] = 'video_Price';
        $data['page_title'] = 'Purchase this video to watch';
        $data['video'] = $this->db->where('videos_id', $param1)->get('videos')->row();
        //$this->load->view('front_end/index', $data);
        $this->load->view('theme/' . $this->active_theme . '/index', $data);
    }
    ...
    function cancel_video_subscription() {
        $response = array();
        $subscription_id = trim($_POST["id"]);
        $response['submitted_data'] = $_POST;
        $status = $this->process_cancel_video_subscription($subscription_id);
        $response['status'] = $status;
        echo json_encode($response);
    }

    function process_cancel_video_subscription($subscription_id = "") {
        $user_id = $this->session->userdata('user_id');
        $query = $this->db->get_where('videos_purchased', array('id' => $subscription_id, 'user_id' => $user_id));
        if ($user_id == '' || $user_id == NULL) {
            return 'login_error';
        } else if ($query->num_rows() > 0) {
            $vide_purchased_data = $query->row();
            ...
            $stripe_data['customer_id'] = $vide_purchased_data->customer_id;
            $stripe_data['transaction_id'] = $vide_purchased_data->transaction_id;
            $stripe_response = $this->stripegateway->cancel_plan($stripe_data);
            if ($stripe_response->status == 'canceled') {
                $data['current_period_end'] = date('Y-m-d H:i:s', $stripe_response->current_period_end);
                $data['status'] = $stripe_response->status;
                $this->db->where('id', $subscription_id);
                $this->db->update('videos_purchased', $data);
                return 'success';
            } else {
                return 'error';
            }
    ...
    function stripe($plan_id = '', $video_id = '') {
        if (isset($_POST['stripeToken'])) {
            $is_tvseries = 0;
            if ($video_id != '' && $video_id > 0) {
                $plan_name = $this->db->get_where('videos', array('videos_id' => $video_id))->row()->title;
                $stripe_plan_id = $this->db->get_where('videos', array('videos_id' => $video_id))->row()->stripe_plan_id;
                $is_paid = $this->db->get_where('videos', array('videos_id' => $video_id))->row()->is_paid;
                if ($is_paid == 3) {
                    $is_tvseries = 1;
                }
                $price = $this->db->get_where('videos', array('videos_id' => $video_id))->row()->fixed_price;
            ...
            if ($is_tvseries) {
                // subscribe tv series
                $stripe_data['plan_id'] = $stripe_plan_id;
                $stripe_data['email'] = $this->db->get_where('user', array('user_id' => $this->session->userdata('user_id')))->row()->email;
                $stripe_response = $this->stripegateway->subscribe_plan($stripe_data);
                if ($stripe_response->status == 'active') {
                    // Subscription info 
                    $subscrID = $stripe_response->id;
                    $custID = $stripe_response->customer;
                    $planID = $stripe_response->plan->id;
                    $planAmount = ($stripe_response->plan->amount / 100);
                    $planCurrency = $stripe_response->plan->currency;
                    $planinterval = $stripe_response->plan->interval;
                    $planIntervalCount = $stripe_response->plan->interval_count;
                    $created = date("Y-m-d H:i:s", $stripe_response->created);
                    $current_period_start = date("Y-m-d H:i:s", $stripe_response->current_period_start);
                    $current_period_end = date("Y-m-d H:i:s", $stripe_response->current_period_end);
//                    $status = $subsData['status']; 
                    $data_video['transaction_id'] = $subscrID;
                    $data_video['customer_id'] = $custID;
                    $data_video['payment_info'] = $planID;
                    $data_video['user_id'] = $this->session->userdata('user_id');
                    $data_video['paid_amount'] = $planAmount;
                    $data_video['currency'] = $planCurrency;
                    $data_video['payment_timestamp'] = $created;
                    $data_video['payment_method'] = 'stripe';
                    $data_video['videos_id'] = $video_id;
                    $data_video['current_period_start'] = $current_period_start;
                    $data_video['current_period_end'] = $current_period_end;
                    $data_video['status'] = $stripe_response->status;
                    $this->add_subscriber_commision($data_video);
                    $this->db->insert('videos_purchased', $data_video);
                    $this->session->set_flashdata('success', 'Video purchase successfully!');
                    redirect(base_url('my-account/purchase-videos'), 'refresh');
                } else {
                    $this->session->set_flashdata('error', 'Transaction fail to process.</br>Reason:' . $stripe_response);
                    redirect(base_url('my-account/purchase-videos'), 'refresh');
                }
            } else {
                // regular payment
            ...
            if (isset($stripe_response->paid) && $stripe_response->paid):
                ...
                $data['status'] = 1;
                if ($video_id != '' && $video_id > 0) {
                    $data_video['transaction_id'] = $data['transaction_id'];
                    $data_video['payment_info'] = $data['payment_info'];
                    $data_video['user_id'] = $data['user_id'];
                    $data_video['paid_amount'] = $data['paid_amount'];
                    $data_video['currency'] = $data['currency'];
                    $data_video['payment_timestamp'] = $data['payment_timestamp'];
                    $data_video['payment_method'] = $data['payment_method'];
                    $data_video['videos_id'] = $video_id;
                    $data_video['current_period_start'] = date('Y-m-d H:i:s', $data['payment_timestamp']);
                    $data_video['status'] = 'active';
                    $this->add_subscriber_commision($data_video);
                    $this->db->insert('videos_purchased', $data_video);
                ...
                echo $this->stripegateway->checkout($stripe_data);
    ...
    function add_subscriber_commision($data_video) {
        $price = $data_video['paid_amount'];
        $percentage = 30; // profit percentage
        $price = $price - ($percentage / 100 ) * $price;
        $created_by = $this->db->get_where('videos', array('videos_id' => $data_video['videos_id']))->row()->created_by;
        $balance = $this->db->get_where('user', array('user_id' => $created_by))->row()->balance;
        $newbalance = $balance + $price;
        $this->db->where('user_id', $created_by);
        $this->db->update('user', array('balance' => $newbalance));
        $data['debit'] = $price;
        $data['created_by'] = $data_video['user_id'];
        $data['video_created_by'] = $created_by;
        $data['description'] = 'Comission';
        $data['video_id'] = $data_video['videos_id'];
        $data['created_at'] = $data_video['current_period_start'];
        $this->db->insert('earnings', $data);
        $invited_by_ref_code=$this->db->get_where('user', array('user_id' => $created_by))->row()->invited_by_ref_code;
        if($invited_by_ref_code){
            $user=$this->db->get_where('user', array('referral' => $invited_by_ref_code))->row();
            $price = $data_video['paid_amount'];
            $percentage = 95; // profit percentage
            $price = $price - ($percentage / 100 ) * $price;
            ...
            $data['created_by'] = $data_video['user_id'];
            $data['video_created_by'] = $created_by;
            $data['description'] = 'Refferal bonus from '.$user->name;
            $data['video_id'] = $data_video['videos_id'];
            $data['created_at'] = $data_video['current_period_start'];
             return $this->db->insert('earnings', $data);
        }
    }
    ...
    function paypal($action = '') {
        if ($action == 'process') {
            ...
            if ($video_id != '' && $video_id > 0) {
                $plan_name = $this->db->get_where('videos', array('videos_id' => $video_id))->row()->title;
                $amount = $this->db->get_where('videos', array('videos_id' => $video_id))->row()->fixed_price;
            } else {
                $plan_name = $this->db->get_where('plan', array('plan_id' => $plan_id))->row()->name;
            ...
            $user_id = $this->session->userdata('user_id');
            $custom = 'user_id=' . $user_id . '&plan_id=' . $plan_id . '$video_id=' . $video_id;
