...
class User extends Home_Core_Controller {
    ...
    public function login() {
        ...
        $data['ref_code'] = $this->input->get('ref');
        if ($this->input->get('ref') != '') {
            if ($user = $this->common_model->validate_referral_code($this->input->get('ref'))) {
                $data['referrel_message'] = "You are using referal code: " . $this->input->get('ref');
            } else {
                $data['referrel_message_error'] = 'The refferal code is invalid! you can still signup without a referal code';
                $data['ref_code'] = '';
            }
        }
            ...
            if ($registration_enable == "1"):
                ...
                if ($this->form_validation->run() == FALSE):
                    ...
                    if ($email_exist):
                        ...
                        $data['join_date'] = date('Y-m-d H:i:s');
                        $data['last_login'] = date('Y-m-d H:i:s');
                        $data['invited_by_ref_code'] = $this->input->post('ref_code');
        ...
        if ($query->num_rows() > 0):
            ...
            if ($row->role == 'admin'):
                ...
                if ($row->is_publisher == 1) {
                    $this->session->set_userdata('publisher_is_login', $row->is_publisher);
                }
                $this->session->set_userdata('login_type', 'admin');
            endif;
            if ($row->role == 'subscriber'):
    ...
    function purchase_videos($param1 = '', $param2 = '') {
        $this->session->unset_userdata('user_active_menu');
        $this->session->set_userdata('user_active_menu', '2');
        ...
        $data['page_name'] = 'purchase_videos';
        $data['title'] = 'My purchase videos';
        $data['profile_info'] = $this->db->get_where('user', array('user_id' => $this->session->userdata('user_id')))->row();
        $this->load->view('theme/' . $this->active_theme . '/index', $data);
    }
