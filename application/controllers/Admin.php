...
class Admin extends Admin_Core_Controller {
    ...
    function __construct() {
        ...
        if (!validate_user_menu_for_controller(array($active_menu)) && !in_array($route, $public)) {
    ...
    function dashboard() {
        ...
        $data['page_title'] = trans('my_dashboard');
    ...
    // slider
    function video_slider($param1 = '', $param2 = '') {
        // active menu session
        $this->session->unset_userdata('active_menu');
        $this->session->set_userdata('active_menu', '401');
        ...
        if ($param1 == 'add') {
            ...
            $this->db->insert('video_slider', $data);
            $insert_id = $this->db->insert_id();
            if (isset($_FILES['video']) && $_FILES['video']['name'] != '') {
                $ext = pathinfo($_FILES['video']['name'], PATHINFO_EXTENSION);
                move_uploaded_file($_FILES['video']['tmp_name'], 'uploads/sliders/video-' . $insert_id . '.'.$ext);
                $data['image_link'] = base_url() . 'uploads/sliders/video-' . $insert_id . '.'.$ext;
            }
            $this->db->where('slider_id', $insert_id);
            $this->db->update('video_slider', $data);
            $this->session->set_flashdata('success', 'Slider added successed');
            redirect($this->agent->referrer());
        }
        ...
        if ($param1 == 'update') {
            ...
            if (isset($_FILES['video']) && $_FILES['video']['name'] != '') {
                $ext = pathinfo($_FILES['video']['name'], PATHINFO_EXTENSION);
                move_uploaded_file($_FILES['video']['tmp_name'], 'uploads/sliders/video-' . $param2 . '.'.$ext);
                $data['image_link'] = base_url() . 'uploads/sliders/video-' . $param2 . '.'.$ext;
            }
            $data['slug'] = url_title($this->input->post('title'), 'dash', TRUE);
            $data['publication'] = $this->input->post('publication');
        ...
        $data['page_name'] = 'video_slider';
        $data['page_title'] = 'Video Slider Management';
        $this->db->order_by('order', "DESC");
        $data['sliders'] = $this->db->get('video_slider')->result_array();
        $this->load->view('admin/index', $data);
    }
    ...
    function videos_edit($param1 = '', $param2 = '') {
        ...
        $data['param1'] = $param1;
        $data['param2'] = $param2;
        $data['users'] =  $this->common_model->get_users_for_video_assigne();
    ...
    function videos($param1 = '', $param2 = '') {
        ...
        if ($param1 == 'add') {
            ...
            $data['fixed_price'] = $this->input->post('fixed_price');
            ...
            $tmdb_json = file_get_contents('https://api.themoviedb.org/3/movie/' . $this->input->post('imdbid') . '/videos?api_key=f915c85186164dea375ae5e2b5312f7f&language=en-US');
            $tmdb = array();
            ...
            foreach ($results as $data) {
                array_push($movieids, $data['key']);
                array_push($movieidsx, $data['name']);
            }


            if (is_null($movieidsx[0])) {
                $movieidsx[0] = "TRAILER";
            }

            if (is_null($movieids[0])) {
                $movieids[0] = "qolmz4FlnZ0";
            }
            ...
            $movie_datax['stream_key'] = $randomString;
            $movie_datax['videos_id'] = $insert_id;
            $movie_datax['file_source'] = 'youtube';
            $movie_datax['source_type'] = 'link';
            $movie_datax['file_url'] = "https://www.youtube.com/watch?v=" . $movieids[0];
            $movie_datax['label'] = $movieidsx[0];
            $movie_datax['order'] = 1;
            $this->db->insert('video_file', $movie_datax);

            $random_number_srv = rand(1, 100);
            ...
            $movie_datax['stream_key'] = $randomString;
            $movie_datax['videos_id'] = $insert_id;
            $movie_datax['file_source'] = 'embed';
            $movie_datax['source_type'] = 'link';
            $movie_datax['file_url'] = "https://fsapi.xyz/movie/" . $this->input->post('imdbid');
            $movie_datax['label'] = 'FS SERVER EN (Mobile&PC)';
            $movie_datax['order'] = 1;
            $this->db->insert('video_file', $movie_datax);
            ...
            $movie_datax['file_source'] = 'embed';
            $movie_datax['source_type'] = 'link';
            $movie_datax['file_url'] = "https://v2.vidsrc.me/embed/" . $this->input->post('imdbid');
            $movie_datax['label'] = 'VidSrc SERVER EN - (Mobile&PC)';
            $movie_datax['order'] = 1;
            $this->db->insert('video_file', $movie_datax);
            ...
            $movie_datax['file_source'] = 'embed';
            $movie_datax['source_type'] = 'link';
            $movie_datax['file_url'] = "https://api.123movie.cc/jadeed.php?imdb=" . $this->input->post('imdbid') . "&server_name=vcu";
            $movie_datax['label'] = '123movie SERVER EN - (Mobile&PC)';
            $movie_datax['order'] = 1;
            $this->db->insert('video_file', $movie_datax);
            ...
            $movie_datax['file_source'] = 'embed';
            $movie_datax['source_type'] = 'link';
            $movie_datax['file_url'] = "https://eb2.srtaem.casa/embed/" . $this->input->post('imdbid');
            $movie_datax['label'] = 'Vikv SERVER EN - (Mobile&PC)';
            $movie_datax['order'] = 1;
            $this->db->insert('video_file', $movie_datax);
            ...
            $movie_datax['stream_key'] = $randomString;
            $movie_datax['videos_id'] = $insert_id;
            $movie_datax['file_source'] = 'm3u8';
            $movie_datax['source_type'] = 'link';
            $movie_datax['file_url'] = "https://srv" . $random_number_srv . ".filmmodu.xyz/" . $this->input->post('imdbid') . "/1080.en.m3u8";
            $movie_datax['label'] = 'VIP SERVER 1080p EN - Mobile';
            $movie_datax['order'] = 1;
            $this->db->insert('video_file', $movie_datax);
            ...
            $movie_datax['file_source'] = 'm3u8';
            $movie_datax['source_type'] = 'link';
            $movie_datax['file_url'] = "https://srv" . $random_number_srv . ".filmmodu.xyz/" . $this->input->post('imdbid') . "/720p.en.m3u8";
            $movie_datax['label'] = 'VIP SERVER 720p EN - Mobile';
            $movie_datax['order'] = 1;
            $this->db->insert('video_file', $movie_datax);
            ...
            $movie_datax['file_source'] = 'm3u8';
            $movie_datax['source_type'] = 'link';
            $movie_datax['file_url'] = "https://srv" . $random_number_srv . ".filmmodu.xyz/" . $this->input->post('imdbid') . "/480p.en.m3u8";
            $movie_datax['label'] = 'VIP SERVER 480p EN - Mobile';
            $movie_datax['order'] = 1;
            $this->db->insert('video_file', $movie_datax);
            ...
            $movie_datax['file_source'] = 'embed';
            $movie_datax['source_type'] = 'link';
            $movie_datax['file_url'] = "https://sinefy.net/api/play/" . $this->input->post('imdbid') . "/en";
            $movie_datax['label'] = 'Sinefy SERVER EN - (Mobile&PC)';
            $movie_datax['order'] = 1;
            $this->db->insert('video_file', $movie_datax);
            ...
            $movie_datax['file_source'] = 'embed';
            $movie_datax['source_type'] = 'link';
            $movie_datax['file_url'] = "https://sinefy.net/api/play/" . $this->input->post('imdbid') . "/tr";
            $movie_datax['label'] = 'Sinefy SERVER TR - (Mobile&PC)';
            $movie_datax['order'] = 1;
            $this->db->insert('video_file', $movie_datax);
            ...
            $movie_datax['file_source'] = 'm3u8';
            $movie_datax['source_type'] = 'link';
            $movie_datax['file_url'] = "https://srv" . $random_number_srv . ".filmmodu.xyz/" . $this->input->post('imdbid') . "/1080.tr.m3u8";
            $movie_datax['label'] = 'VIP SERVER 1080p TR - Mobile';
            $movie_datax['order'] = 1;
            $this->db->insert('video_file', $movie_datax);
            ...
            $movie_datax['file_source'] = 'm3u8';
            $movie_datax['source_type'] = 'link';
            $movie_datax['file_url'] = "https://srv" . $random_number_srv . ".filmmodu.xyz/" . $this->input->post('imdbid') . "/720p.tr.m3u8";
            $movie_datax['label'] = 'VIP SERVER 720p TR - Mobile';
            $movie_datax['order'] = 1;
            $this->db->insert('video_file', $movie_datax);
            ...
            $movie_datax['file_source'] = 'm3u8';
            $movie_datax['source_type'] = 'link';
            $movie_datax['file_url'] = "https://srv" . $random_number_srv . ".filmmodu.xyz/" . $this->input->post('imdbid') . "/480p.tr.m3u8";
            $movie_datax['label'] = 'VIP SERVER 480p TR - Mobile';
            $movie_datax['order'] = 1;
            $this->db->insert('video_file', $movie_datax);
        ...
        if ($this->session->userdata('publisher_is_login') == 1) {
    ...
    function tvseries($param1 = '', $param2 = '') {
        ...
        if ($param1 == 'add') {
            ...
            if ($this->input->post('is_paid') == 3 && $this->input->post('fixed_price') != '' && $this->input->post('fixed_price') > 0) {
                ...
                $stripe_data['amount'] = $this->input->post('fixed_price');
                $stripe_data['currency'] = strtolower($currency_code);
                $stripe_data['title'] = $this->input->post('title');
                $stripe_data['stripe_secret_key'] = $stripe_secret_key;
                $stripe_data['stripe_token'] = $stripe_secret_key;
                $stripe_response = $this->stripegateway->create_plan($stripe_data);
                if (isset($stripe_response->id) && $stripe_response->id) {
                    $data['stripe_plan_id'] = $stripe_response->id;
                } else {
                    die('unable to create stripe plan id');
                }
            }
            ...
            if ($this->input->post('is_paid') == 3 && $this->input->post('fixed_price') != '' && $this->input->post('fixed_price') > 0) {
                ...
                $stripe_data['stripe_secret_key'] = $stripe_secret_key;
                $stripe_data['stripe_token'] = $stripe_secret_key;
                $stripe_plan_id = $this->db->get_where('videos', array('videos_id' => $param2))->row()->stripe_plan_id;
                if ($stripe_plan_id == '') {
                    $stripe_response = $this->stripegateway->create_plan($stripe_data);
                    if (isset($stripe_response->id) && $stripe_response->id) {
                        $data['stripe_plan_id'] = $stripe_response->id;
    ...
    function manage_live_tv($param1 = '', $param2 = '') {
        ...
        if ($param1 == 'new') {
            ...
            $filter = array();
            if ($title != "" && $title != NULL) {
                $filter['tv_name'] = $title;
                $search_string .= 'tv_name=' . $title . '';
                $data['title'] = $title;
            }
            $total_rows = $this->live_tv_model->num_live_tv($filter);
            $config = $this->common_model->pagination();
            ...
            $data['last_row_num'] = $this->uri->segment(3);
            $page = $this->input->get('per_page'); //($this->uri->segment(3)) ? $this->uri->segment(3) : 0;   
            $data["tvs"] = $this->live_tv_model->get_live_tvs($filter, $config["per_page"], $page);
            $data["links"] = $this->pagination->create_links();
            $data['total_rows'] = $config["total_rows"];
    ...
    function earnings($param1 = '', $param2 = '') {
        if ($this->session->userdata('admin_is_login') != 1)
            redirect(base_url(), 'refresh');
        /* start menu active/inactive section */
        $this->session->unset_userdata('active_menu');
        $this->session->set_userdata('active_menu', '1502');
        ...
        $total_rows = $this->common_model->get_user_earnings_num_rows($name);
        // page
        $config = $this->common_model->pagination();
        $config["base_url"] = base_url() . "admin/earnings?" . $search_string;
        $config["total_rows"] = $total_rows;
        $config["per_page"] = 15;
        $config["uri_segment"] = 3;
        //$config['use_page_numbers'] = TRUE;
        ...
        $data["earnings"] = $this->common_model->get_user_earnings($name, $config["per_page"], $page);
        $data["links"] = $this->pagination->create_links();
        $data['total_rows'] = $config["total_rows"];
        $data['balance'] = $this->db->get_where('user', array('user_id' => $this->session->userdata('user_id')))->row()->balance;
        $data['stripe_account_id'] = $this->db->get_where('user', array('user_id' => $this->session->userdata('user_id')))->row()->stripe_account_id;
        $data['page_name'] = 'earnings';
        $data['page_title'] = trans('Earnings');
        $this->load->view('admin/index', $data);
    }
     function referrals($param1 = '', $param2 = '') {
        if ($this->session->userdata('admin_is_login') != 1)
            redirect(base_url(), 'refresh');
        /* start menu active/inactive section */
        $this->session->unset_userdata('active_menu');
        $this->session->set_userdata('active_menu', '1503');
        
        $referral=$this->db->get_where('user', array('user_id' => $this->session->userdata('user_id')))->row()->referral;
        if(!isset($referral) && $referral==''){
           $referral= $this->common_model->create_new_referral_code();
        }
        ...
        $total_rows = $this->common_model->get_user_referrals_num_rows($name,$referral);
        // page
        $config = $this->common_model->pagination();
        $config["base_url"] = base_url() . "admin/referrals?" . $search_string;
        $config["total_rows"] = $total_rows;
        $config["per_page"] = 15;
        $config["uri_segment"] = 3;
        //$config['use_page_numbers'] = TRUE;
        ...
        $data['last_row_num'] = $this->uri->segment(3);
        $page = ($this->input->get('per_page') != "" || $this->input->get('per_page') != NULL) ? $this->input->get('per_page') : 0; //($this->uri->segment(3)) ? $this->uri->segment(3) : 0;   
        $data["referrals"] = $this->common_model->get_user_referrals($name, $config["per_page"], $page,$referral);
        $data["links"] = $this->pagination->create_links();
        $data['total_rows'] = $config["total_rows"];
        $data['page_name'] = 'referrals';
        $data['page_title'] = trans('referrals');
        $this->load->view('admin/index', $data);
    }
    ...
    function stripeconnect() {
        ...
        $stripe_secret_key = $this->db->get_where('config', array('title' => 'stripe_secret_key'))->row()->value;
        $stripe_data['stripe_secret_key'] = $stripe_secret_key;
        $stripe_data['user']=$this->db->get_where('user', array('user_id' => $this->session->userdata('user_id')))->result_array()[0];
        $account_id=$this->stripegateway->stripe_add_customer($stripe_data);
        $this->db->where('user_id',$this->session->userdata('user_id'))->update('user', array('stripe_account_id' => $account_id));
        $stripe_data['account_id'] = $account_id;
        $url=$this->stripegateway->stripe_connect($stripe_data);
        header('location:'.$url);
    }
    function stripepayout() {
        $stripe_secret_key = $this->db->get_where('config', array('title' => 'stripe_secret_key'))->row()->value;
        $stripe_data['stripe_secret_key'] = $stripe_secret_key;
        $stripe_data['balance'] =$price= $this->db->get_where('user', array('user_id' => $this->session->userdata('user_id')))->row()->balance;
        $stripe_data['stripe_account_id'] = $this->db->get_where('user', array('user_id' => $this->session->userdata('user_id')))->row()->stripe_account_id;
        if($stripe_data['balance']>=30){
        $res=$this->stripegateway->stripe_payout($stripe_data);
        if($res){
            $this->db->where('user_id',$this->session->userdata('user_id'))->update('user', array('balance' => 0));
            $data['credit'] = $price;
            $data['created_by'] = $this->session->userdata('user_id');
            $data['video_created_by'] = $this->session->userdata('user_id');
            $data['description'] = 'payout';
            $data['video_id'] = '';
            $data['created_at'] = date('Y-m-d H:i:s');
            $this->db->insert('earnings', $data);
            $this->session->set_flashdata('success', 'Pay out sent to your connected account');
        }else{
           $this->session->set_flashdata('error', 'something went wrong'); 
        }
        }else{
            $this->session->set_flashdata('error', 'Atleast $30 required for payout'); 
        }
       redirect($this->agent->referrer());
            //header('location:'.$url);
    }

    function returnstripeconnect() {
        if (isset($_GET['code'])) {
            echo $_GET['code'];
        }
        $this->session->set_flashdata('success', 'Stripe Account connected successfully');
        redirect('earnings');
    }

    // users
    function user_role_manage($param1 = '', $param2 = '') {
        if ($this->session->userdata('admin_is_login') != 1)
            redirect(base_url(), 'refresh');
        /* start menu active/inactive section */
        $this->session->unset_userdata('active_menu');
        $this->session->set_userdata('active_menu', '1501');
        /* end menu active/inactive section */
        ...
        if ($param1 == 'add') {
            ...
            $data['is_allowed'] = $this->input->post('is_allowed');
            $data['created_at'] = date('Y-m-d H:i:s');
        ...
        if ($param1 == 'update') {
            ...
            $data['updated_at'] = date('Y-m-d H:i:s');
            $data['updated_by'] = 1;
            $this->db->where('user_permissions_id', $param2);
        ...
        $total_rows = $this->common_model->get_user_role_permissions_num_rows($name);
        // page
        $config = $this->common_model->pagination();
        $config["base_url"] = base_url() . "admin/user_role_manage?" . $search_string;
        $config["total_rows"] = $total_rows;
        $config["per_page"] = 15;
        $config["uri_segment"] = 3;
        //$config['use_page_numbers'] = TRUE;
        ...
        $data['last_row_num'] = $this->uri->segment(3);
        $page = ($this->input->get('per_page') != "" || $this->input->get('per_page') != NULL) ? $this->input->get('per_page') : 0; //($this->uri->segment(3)) ? $this->uri->segment(3) : 0;   
        $data["users"] = $this->common_model->get_user_role_permissions($name, $config["per_page"], $page);
        $data["links"] = $this->pagination->create_links();
        $data['total_rows'] = $config["total_rows"];
        $data['page_name'] = 'user_role_manage';
        $data['page_title'] = 'User Role Managment';
        $this->load->view('admin/index', $data);
    }
    ...
    function manage_user($param1 = '', $param2 = '') {
        ...
        if ($param1 == 'add') {
            if ($this->db->get_where('user', array('email' => $this->input->post('email')))->num_rows() > 0) {
                ...
                if ($this->input->post('role') == 'publisher') {
                    $role = 'admin';
                    $data['is_publisher'] = 1;
                }
                $data['role'] = $role;
    ...
    function profile($param1 = '', $param2 = '', $param3 = '') {
        ...
        if ($param1 == 'update') {
            $data['name'] = $this->input->post('name');
            $data['email'] = $this->input->post('email');
            $data['invited_by_ref_code'] =$refferal= $this->input->post('invited_by_ref_code');
            if($data['invited_by_ref_code']!=''){
            $user=$this->db->get_where('user', array('referral' => $refferal))->row();
            if(!$user){
                $this->session->set_flashdata('error', 'You have entered an invalid referal code');
                redirect($this->agent->referrer());
            }
            }
    ...
    function delete_record() {
        ...
        $table_row_id = $this->input->post('primary_id');
        if($table_row_id==''){
