...
class Watch extends Home_Core_Controller{
    public function index($slug='',$param1='',$param2=''){
        ...
        if($error == FALSE):            
            $data['videos_id'] =$video_id     = $this->common_model->get_videos_id_by_slug($slug);
            ...
            elseif($accessibility =="video_denied"):
                $this->session->set_flashdata('error', 'Please Purchase this video to watch');
                redirect(base_url('subscription/purchase/'.$video_id), 'refresh');
            endif;
