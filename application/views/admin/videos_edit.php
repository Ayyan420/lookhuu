...
foreach ($videos as $video) :
    ...
    echo form_open(base_url() . 'admin/videos/update/' . $param1, array('class' => 'form-horizontal group-border-dashed', 'enctype' => 'multipart/form-data'));
     if($this->session->userdata('publisher_is_login') == 1 && $video['created_by']!=$this->session->userdata('user_id')){
                      ...
                      <label class="control-label">Free/Paid</label>
                        <select  class="form-control" onchange="show_fixed_price_input(this.value)"  name="is_paid">
                          ...
                          <option value="1" <?php if($video['is_paid'] =='1'){ echo "selected";} ?>>Paid</option>
                            <option value="2" <?php if($video['is_paid'] =='2'){ echo "selected";} ?>>Fixed Price</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <input type="text" class="form-control fixed_price" value="<?=$video['fixed_price']?>"  name="fixed_price" style=" <?php if($video['is_paid'] !='2'){ echo "display:none";} ?>" placeholder="Enter the price">
                    </div>
                    <div class="row">
