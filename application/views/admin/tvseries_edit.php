...
foreach ($videos as $video) :
                ...
                <div class="card-block">
                    ...
                    <div class="form-group">
                      <label class="control-label">Free/Paid</label>
                        <select  class="form-control" onchange="show_fixed_price_input(this.value)"  name="is_paid">
                          <option value="0"  <?php if($video['is_paid'] =='0'){ echo "selected";} ?>>Free</option>
                          <option value="1" <?php if($video['is_paid'] =='1'){ echo "selected";} ?>>For website subscribers</option>
                          <option value="2" <?php if($video['is_paid'] =='2'){ echo "selected";} ?>>One time payment</option>
                          <option value="2" <?php if($video['is_paid'] =='3'){ echo "selected";} ?>>Monthly Subscription</option>
                        </select>
                      
                    </div>
                     <div class="form-group">
                        <input type="text" class="form-control fixed_price" value="<?=$video['fixed_price']?>"  name="fixed_price" style=" <?php if($video['is_paid'] !='2' || $video['is_paid'] !='3'){ echo "display:none";} ?>" placeholder="Enter the price">
                    </div>
 ...
 <script>
        function show_fixed_price_input(val){
            if(val==2 || val==3){
                $(".fixed_price").show();
            }else{
                $(".fixed_price").hide();
        ...
        <div class="col-md-6">
            <div class="card cta cta--featured p-a">
                ...
                <?php echo $video['meta_description']; ?>
                    ...
                    <?php if($this->session->userdata('publisher_is_login') != 1): ?>
                    <div class="form-group">
                        <label class="control-label"><?php echo trans('Assign video to other user'); ?></label>
                        <select name="created_by" class="form-control" required="">
                            <option value="">Created By</option>
                            <?php foreach ($users as $key => $value) {
                                $selected=$video['created_by']==$value->user_id?'selected':'';
