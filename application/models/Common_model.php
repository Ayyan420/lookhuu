...
class Common_model extends CI_Model {
    ...
    public function get_user_role_permissions_num_rows($name='')
    {
        if($name !="" && $name !=NULL):
            $this->db->like("menu_name",$name,"both");
        endif;
        return $this->db->get('user_permissions')->num_rows();
    }
    public function get_user_earnings_num_rows($name='')
    {
        return $this->db->get('earnings')->num_rows();
    }
    public function get_user_referrals_num_rows($name='',$referral='')
    {
        $this->db->where('invited_by_ref_code',$referral);
        return $this->db->get('user')->num_rows();
    }
    ...
    public function get_users_for_video_assigne()
    {
        $this->db->where('role','admin');
        return $this->db->get('user')->result();
    }
    public function get_user_role_permissions($name='',$limit=NULL, $start=NULL)
    {
        if($name !="" && $name !=NULL):
            $this->db->like("menu_name",$name,"both");
        endif;
        $this->db->order_by('is_allowed','desc');
        $this->db->limit($limit,$start);
        return $this->db->get('user_permissions')->result_array();
    }
    public function get_user_earnings($name='',$limit=NULL, $start=NULL)
    {
        $this->db->where('video_created_by',$this->session->userdata('user_id'));
        $this->db->limit($limit,$start)->order_by('id','desc');
        return $this->db->get('earnings')->result_array();
    }
    public function get_user_referrals($name='',$limit=NULL, $start=NULL,$referral)
    {
        $this->db->where('invited_by_ref_code',$referral);
        $this->db->limit($limit,$start)->order_by('join_date','desc');
        return $this->db->get('user')->result_array();
    }
    
    public function create_new_referral_code($refferal='') {
        while($refferal==''){
            $rand='';
            $id= $this->session->userdata('user_id');
            $name= $this->db->get_where('user', array('user_id' => $id))->row()->name;
            $refferal=clean($name.$id.$rand);
            $user=$this->db->get_where('user', array('referral' => $refferal))->row();
            if($user){
                $rand= rand(1, 100);
                $refferal='';
            }else{
                $refferal=$this->db->where('user_id', $id)->update('user', array('referral' => $refferal));
            }
        }
        return $refferal;
    }
    public function validate_referral_code($refferal='') {
         return $this->db->get_where('user', array('referral' => $refferal))->row();
    }
    function validate_user_menu($where){
        $this->db->where('is_allowed', 1);
        $this->db->where_in('menu_id', $where);
    ...
    public function all_published_video_slider()
    {
        $this->db->order_by("order","ASC");
        return $this->db->get_where('video_slider', array('publication'=> '1'), 8)->result();
    }
     ...
     public function get_videos($array=array(),$limit=NULL, $start=NULL)
    {
        ...
        public function get_tv_series($array=array(),$limit=NULL, $start=NULL)
    {
        if($array !='' && $array !=NULL && !empty($array))
            $this->db->like($array);
