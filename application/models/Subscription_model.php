...
class Subscription_model extends CI_Model {
    ...
    function check_video_accessibility($videos_id = '') {
        ...
        elseif ($is_paid == '2'):
            // send if user hasnt purchased this video yet
            $accessibility = "video_denied";
            $subscription = $this->check_validated_purchased_videos($videos_id);
            if ($subscription == "login_required"):
                $accessibility = "login_required";
            elseif ($subscription === "TRUE"):
                $accessibility = "allowed";
    ...
    function check_validated_purchased_videos($videos_id) {
        $validity = "FALSE";
        $user_id = $this->session->userdata('user_id');
        if (!empty($user_id)):
            $this->db->where('videos_purchased.status', 'active');
            $this->db->where('videos_id', $videos_id);
            $this->db->where('user_id', $this->session->userdata('user_id'));
            $query = $this->db->get('videos_purchased');
            if ($query->num_rows() > 0):
                $data = $query->row();
                $validity = $data->id;
                ...
                if ($validity) {
                    ...
                    if ($current_period_end != '') {
                        ...
                        $stripe_data['transaction_id'] = $data->transaction_id;
                        $stripe_response = $this->stripegateway->check_subscription($stripe_data);
                        if (isset($stripe_response->id) && $stripe_response->id) {
                            $data_video['current_period_end'] =$current_period_end= date('Y-m-d H:i:s', $stripe_response->current_period_end);
                            $data_video['status'] = $stripe_response->status;
                            $this->db->where('id', $validity);
                            $this->db->update('videos_purchased', $data_video);
                            //check if current period is less then the current date
                            if ($current_period_end < date('Y-m-d H:i:s')) {
                                $validity = "FALSE";
                            }
                        } else {
                            die('unable to check stripe subscription ' . $stripe_response);
                        }
                    }
                }
            endif;
    ...
    function get_videos_purchased() {
        $this->db->where('user_id', $this->session->userdata('user_id'));
        $this->db->join('videos', 'videos.videos_id=videos_purchased.videos_id');
        return $this->db->order_by('id', 'desc')->get('videos_purchased');
    }
